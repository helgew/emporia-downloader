...
class AuthenticationHelper {
    ...
    private byte[] getPasswordAuthenticationKey(String userId, String userPassword, BigInteger B,
            BigInteger salt) {
        ...
        BigInteger S =
                (B.subtract(k.multiply(g.modPow(x, N))).modPow(a.add(u.multiply(x)), N)).mod(N);
     ...
     */
    RespondToAuthChallengeResult performSRPAuthentication(String username, String password) throws Exception {
        ...
        if (ChallengeNameType.PASSWORD_VERIFIER.toString()
            ...
            result = cognitoIdentityProvider.respondToAuthChallenge(challengeRequest);
        }
        return result;
    }
    ...
    private RespondToAuthChallengeRequest userSrpAuthRequest(InitiateAuthResult challenge,
        ...
        try {
            ...
            byte[] secretBlock =
                    Base64.decode(challenge.getChallengeParameters().get("SECRET_BLOCK"));
            mac.update(secretBlock);
            String dateString = formatTimestamp.format(timestamp);
